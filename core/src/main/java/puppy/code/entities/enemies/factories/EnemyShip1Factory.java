...
import puppy.code.entities.enemies.EnemyShip1;
...
public class EnemyShip1Factory extends EnemyFactory {
    
    @Override
    protected Enemy createEnemy(float x, float y, float velocityX, float velocityY, int round) {
        Texture texture = ResourceManager.getInstance().getTexture("Game/Enemys/EnemyShips/EnemyShip1.png");
        int health = 10 + (round * 3);
        return new EnemyShip1(x, y, texture, health, Math.abs(velocityY));
    }
